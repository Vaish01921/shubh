...
# Multiprocess depot startup: submit 2 workers at a time, then sleep 0.5s before next pair (orchestrator-only).
_BATCH_SIZE = 2
_BATCH_GAP_SECONDS = 0.5
...
def _submit_workers_in_batches(
    ...
    """Submit run_depot_worker in batches of _BATCH_SIZE; sleep _BATCH_GAP_SECONDS between batches only."""
    future_map: Dict[Future, str] = {}
    n = len(depot_list)
    for batch_start in range(0, n, _BATCH_SIZE):
        batch = depot_list[batch_start : batch_start + _BATCH_SIZE]
        for d in batch:
            payload = {
                "truck_id": truck_id,
                "depot_name": d,
            ...
            future_map[fut] = d
            logger.info(
                "[Orchestrator] Submitted depot=%s batch_index=%d batch_size=%d (of %d depots)",
                d,
                batch_start // _BATCH_SIZE + 1,
                len(batch),
                n,
            )
        if batch_start + _BATCH_SIZE < n:
            logger.info(
                "[Orchestrator] Inter-batch delay %.1fs before next batch (batch_size=%d)",
                _BATCH_GAP_SECONDS,
                _BATCH_SIZE,
            )
            time.sleep(_BATCH_GAP_SECONDS)
    return future_map
...
def run_multiprocess_job_sync(
    ...
    max_workers = max(1, min(mp_cfg["max_parallel_workers"], len(depot_list)))
    ...
    logger.info(
        ...
        "parent_pid=%s depots=%d max_workers=%s (extra depots queue in pool)",
        os.getpid(),
        len(depot_list),
        max_workers,
    )
    ...
    with ProcessPoolExecutor(max_workers=max_workers, mp_context=ctx) as pool:
        future_map = _submit_workers_in_batches(
            pool,
            depot_list,
            truck_id,
            desired_bids,
...
def _async_job_thread(
    ...
    max_workers = max(1, min(int(mp_cfg["max_parallel_workers"]), len(depot_list)))
    timeout = mp_cfg.get("depot_timeout_seconds")
    timeout_f = float(timeout) if timeout is not None else None
    ctx = mp.get_context("spawn")
    ...
    try:
        logger.info(
            "[Orchestrator] Async job_id=%s depots=%d max_workers=%d",
            job_id,
            len(depot_list),
            max_workers,
        )
