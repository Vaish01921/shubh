...
from .successful_bids_store import insert_successful_bid
...
class DepotBiddingEngine:
    ...
    def _read_row_bid_rank_adjacent(self, row_index: int) -> Optional[str]:
        """
        Read Bid Rank from the <td> immediately right of the Bid Amount cell on the same row.
        Anchors on idBidAmount for the given row_index (same binding SAP uses for bid entry).
        Never raises.
        """
        try:
            bid_id = self._shubh_bid_input_id(int(row_index))
            script = (
                "var bid=document.getElementById(arguments[0]);"
                "if(!bid)return null;"
                "var td=bid.closest('td');"
                "if(!td)return null;"
                "var tr=td.parentElement;"
                "if(!tr)return null;"
                "var tds=tr.querySelectorAll('td');"
                "var bidIdx=-1;"
                "for(var i=0;i<tds.length;i++){if(tds[i]===td){bidIdx=i;break;}}"
                "if(bidIdx<0||bidIdx+1>=tds.length)return null;"
                "var rankTd=tds[bidIdx+1];"
                "var t=(rankTd.innerText!=null?rankTd.innerText:"
                "(rankTd.textContent!=null?rankTd.textContent:''));"
                "t=(t||'').trim();"
                "return t||null;"
            )
            raw = self.driver.execute_script(script, bid_id)
            if raw is None:
                return None
            s = str(raw).strip()
    ...
    def _poll_row_bid_rank_adjacent(
        self,
        row_index: int,
        max_wait: float = 8.0,
        poll_interval: float = 0.3,
    ) -> Optional[str]:
        """Poll adjacent-td rank read until SAP populates the cell after Save/Yes/OK."""
        deadline = time.monotonic() + max_wait
        last: Optional[str] = None
        while time.monotonic() < deadline:
            last = self._read_row_bid_rank_adjacent(row_index)
            if last:
                return last
            time.sleep(poll_interval)
        return last

    def _successful_bid_context_for_row(
        self, planned: List[Dict[str, Any]], row_index: int
    ) -> Optional[Dict[str, Any]]:
        """Resolve destination, quantity, bid_amount from existing plan + desired_bids (no extra cache)."""
        plan_row: Optional[Dict[str, Any]] = None
        for p in planned:
            if p.get("row_index") is None:
                continue
            if int(p["row_index"]) == int(row_index):
                plan_row = p
                break
        if plan_row is None:
            return None
        bid_price = plan_row.get("bid_price")
        if bid_price is None:
            return None
        try:
            bid_amount = int(bid_price)
        except (TypeError, ValueError):
            return None
        dest_raw = plan_row.get("destination")
        destination = str(dest_raw).strip() if dest_raw is not None else ""
        destination = destination or None

        quantity: Optional[float] = None
        for db in self.desired_bids:
            db_dest = getattr(db, "destination", None)
            db_dest_s = str(db_dest).strip() if db_dest is not None else ""
            if destination and db_dest_s:
                if db_dest_s.upper() != destination.upper():
                    continue
            quantity = float(db.quantity)
            break
        if quantity is None and self.desired_bids:
            quantity = float(self.desired_bids[0].quantity)
        if quantity is None:
            return None
        return {
    ...
    def _persist_successful_bid_after_rank_one(
        self,
        prefix: str,
        planned: List[Dict[str, Any]],
        row_index: int,
    ) -> None:
        """Insert SQLite row after confirmed Rank==1. Never raises."""
        try:
            ctx = self._successful_bid_context_for_row(planned, row_index)
            if not ctx:
                logger.warning(
                    "%s [RANK][PERSIST] Missing plan context for row=%s; skip store",
                    prefix,
                    row_index,
                )
                return
            row_id = insert_successful_bid(
                depot=self.depot_name or "UNKNOWN",
                destination=ctx["destination"],
                quantity=float(ctx["quantity"]),
                bid_amount=int(ctx["bid_amount"]),
            )
            if row_id is None:
                logger.warning(
                    "%s [RANK][PERSIST] Failed to store successful bid row=%s",
                    prefix,
                    row_index,
                )
            else:
                logger.info(
                    "%s [RANK][PERSIST] Stored successful bid id=%s depot=%s row=%s",
                    prefix,
                    row_id,
                    self.depot_name,
                    row_index,
            ...
            logger.warning(
                "%s [RANK][PERSIST] Unexpected error storing row=%s: %s",
                prefix,
                row_index,
                e,
            )

    def _log_post_execute_rank_result(
        self,
        prefix: str,
        planned: List[Dict[str, Any]],
        processed_rows: set[int],
    ) -> None:
        """
        Read post-bid Bid Rank from the <td> adjacent to Bid Amount (same <tr> as idBidAmount).
        No Search click; no fixed column index. Never raises.
        """
        depot_label = self.depot_name or "UNKNOWN"
        if not processed_rows:
            return

        for row_index in sorted(processed_rows):
            rank_raw = self._poll_row_bid_rank_adjacent(row_index)
            if not rank_raw:
                logger.warning(
                    "%s [RANK] Bid Rank not found for Depot=%s Row=%s "
                    "(adjacent cell empty or idBidAmount row anchor missing)",
                    prefix,
                    depot_label,
                    row_index,
                )
                continue
            logger.info(
                "%s [RANK] Depot=%s Row=%s BidRank=%s",
                prefix,
                depot_label,
                row_index,
                rank_raw,
            ...
            try:
                ...
                logger.warning(
                    "%s [RANK] Could not parse Bid Rank value %r for Row=%s",
                    prefix,
                    rank_raw,
                    row_index,
                )
                continue
            if rank_int == 1:
                logger.info("%s [RANK][SUCCESS] Bid won.", prefix)
                self._persist_successful_bid_after_rank_one(prefix, planned, row_index)
            else:
                logger.info("%s [RANK] Current Rank = %s", prefix, rank_int)
    ...
    def _execute_fast_bidding(self, planned: List[Dict[str, Any]], prefix: str) -> Dict[str, Any]:
        ...
        if ok_status:
            self._log_post_execute_rank_result(prefix, planned, processed_rows)
        return {
